...
export class WishlistController {
  ...
  @Get()
  @ApiOperation({ summary: 'List wishlist', description: 'Get user wishlist with pagination' })
  ...
  @Post(':carId')
  @ApiOperation({ summary: 'Add to wishlist', description: 'Add a car to user wishlist' })
  add(@Param('carId') carId: string, @Request() req: any) {
    return this.wishlistService.add(req.user.userId, +carId);
  }

  @Delete(':carId')
  @ApiOperation({ summary: 'Remove from wishlist', description: 'Remove a car from user wishlist' })
