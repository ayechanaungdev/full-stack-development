...
export class BookingsController {
    ...
    @Post()
    @ApiOperation({ summary: 'Create a booking', description: 'Book a car for a date range' })
    @ApiResponse({ status: 201, description: 'Booking created successfully' })
    @ApiResponse({ status: 409, description: 'Car not available for the selected dates' })
    ...
    @Get('owner-dashboard')
    @ApiOperation({ summary: 'Owner dashboard', description: 'Get booking stats for car owner dashboard' })
    getOwnerDashboard(@Request() req: any) {
        return this.bookingsService.getOwnerDashboard(req.user.userId);
    }

    @Get('debug')
    @ApiOperation({ summary: 'Debug info', description: 'Get current user info from token (for testing)' })
    getDebugInfo(@Request() req: any) {
        return { userId: req.user.userId, role: req.user.role, email: req.user.email };
    }

    @Get()
    @ApiOperation({ summary: 'List bookings', description: 'Get all bookings with filters (status, date range, search)' })
    findAll(
        @Request() req: any,
    ...
    @Get(':id')
    @ApiOperation({ summary: 'Get booking by ID', description: 'Get booking details (own bookings or admin)' })
    ...
    @Patch(':id/status')
    @ApiOperation({ summary: 'Update booking status', description: 'Update booking status (approve, reject, complete, cancel)' })
    updateStatus(
        @Param('id') id: string,
    ...
    @Patch(':id/read')
    @ApiOperation({ summary: 'Mark booking as read', description: 'Mark a booking notification as read' })
