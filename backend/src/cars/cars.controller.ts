...
export class CarsController {
  ...
  @ApiOperation({ summary: 'Create a car', description: 'Add a new car to the inventory (admin/car_owner)' })
  ...
  @Get()
  @ApiOperation({ summary: 'List cars', description: 'Get paginated list of cars with filters and search' })
  @ApiResponse({ status: 200, description: 'Returns paginated car list with total count' })
  findAll(@Query() query: QueryCarDto) {
    return this.carsService.findAllPaginated(query);
  }

  @Get('price-range')
  @ApiOperation({ summary: 'Get price range', description: 'Get min and max pricePerDay across all cars' })
  getPriceRange() {
    return this.carsService.getPriceRange();
  }

  @Get(':id')
  @ApiOperation({ summary: 'Get car by ID', description: 'Get a single car with full details' })
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(Role.admin, Role.car_owner)
  @ApiOperation({ summary: 'Update a car', description: 'Update car details (admin/car_owner)' })
  @ApiBearerAuth('access-token')
  async update(
    @Param('id') id: string,
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(Role.admin, Role.car_owner)
  @ApiOperation({ summary: 'Delete a car', description: 'Remove a car from inventory (admin/car_owner)' })
  @ApiBearerAuth('access-token')
